...
# GET MOVIES BY GENRE
# -----------------------------
@router.get("/genre/{gen}", status_code=status.HTTP_200_OK)
...
# -----------------------------
@router.get("/rating/{rate}", status_code=status.HTTP_200_OK)
def get_movies_by_rating(rate: float, db: Session = Depends(get_db)):

    movies = db.query(models.Movie).filter(
        models.Movie.rating >= rate
    ).all()
...
def update_movie(id: int, updated_movie: schemas.MovieCreate,
                 db: Session = Depends(get_db)):

    movie_query = db.query(models.Movie).filter(
        models.Movie.movie_id == id
    )
